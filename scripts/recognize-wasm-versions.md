...
After each WASM deploy, increment `scripts/recognize-wasm-version.txt`, append the row in both changelogs, `git tag recognize-wasm-vN`, and:

```
docker tag sysclinicas-onlyoffice:9.2.1-writenew sysclinicas-onlyoffice:recognize-vN
```

Copy `scripts/recognize-wasm-version.txt` into the image at:
